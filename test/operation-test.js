...
const Render = require('../src/util/render')
const {
...
describe('Operation Test', () => {
  before('tear up', async () => {
    ...
    this.operation = new Operation(this.dbPath, new Render({
      info: () => {},
      error: () => {},
      warn: () => {},
      debug: () => {}
  ...
  describe('List Operation Test', () => {
    it('should create list', async () => {
      await this.operation.listOperation({ create: 'testList' })
    ...
    it('should delete list with the name', async () => {
      ...
      expect(lists.length).to.equal(1)
      expect(lists[0].name).to.equal('testList')
      await this.operation.listOperation({ delete: 'testList' })
    ...
    it('should update the list with the new name', async () => {
      ...
      expect(lists.length).to.equal(1)
      expect(lists[0].name).to.equal('testList')
      await this.operation.listOperation({ update: ['testList', 'testList2'] })
      lists = await this.listService.getLists()
      expect(lists.length).to.equal(1)
